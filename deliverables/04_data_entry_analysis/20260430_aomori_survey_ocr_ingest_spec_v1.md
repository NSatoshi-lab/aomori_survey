...
- スキャナーでJPEG化した配布版4ページ回答票を、匿名ID単位の解析CSVへ変換する。
...
- ページ種別は印字内容から判定する。
- 期待するページ種別は `cover`, `page_q1_q5`, `page_q6_q9`, `page_q10_q11` の4種類。
- 1つの `response_id` で4種類が揃わない場合、または同じページ種別が重複する場合は目視確認に回す。
